...
# find largest product of 4 numbers in any direction (up/down,  left/right,  diagonal)
SIZE = 20
...
def findUpDownProd(grid, row, col):
    ...
    prod = 1
    for i in range (0, 4, 1):
        print "findUpDownProd - row=", row + i
        prod *= grid[row+i][col]
    return prod


def findLargestProd(grid):
    maxProd = 0 
...
def main():
    maxProduct = findLargestProd(GRID)
    print "max product in grid is ", maxProduct
